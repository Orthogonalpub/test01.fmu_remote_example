...
string server_ip="192.168.1.32";


/***************************************************
Types for Orth Functions
****************************************************/
string orthEcho(string s ) {
        auto r = client->call("orthEcho", s).as<string>();
            return (r);
}
int orthSetRunStatus(int v ) {
        auto r = client->call("orthSetRunStatus", v).as<int>();
            return (r);
}
int orthSetRunInterval(unsigned long interval, unsigned long stoptime ) {
        auto r = client->call("orthSetRunInterval", interval, stoptime).as<int>();
            return (r);
}
fmi2Status orthFmi2DoStepLoop(fmi2Component c, fmi2Real currentCommunicationPoint, fmi2Real communicationStepSize, fmi2Boolean noSetFMUStatePriorToCurrentPoint) {
        //auto r = client->call("orthFmi2DoStepLoop", double(currentCommunicationPoint), double(communicationStepSize), int(noSetFMUStatePriorToCurrentPoint)).as<ReturnValue>();
            auto afuture = client->async_call("orthFmi2DoStepLoop", double(currentCommunicationPoint), double(communicationStepSize), int(noSetFMUStatePriorToCurrentPoint));
                return fmi2OK;
}
...
fmi2Component fmi2Instantiate(fmi2String instanceName, fmi2Type fmuType, fmi2String fmuGUID, fmi2String fmuResourceLocation, const fmi2CallbackFunctions* functions, fmi2Boolean visible, fmi2Boolean loggingOn) {
    ...
    for (int attempts = 0;; attempts++) {
        try {
            ...
            client = new rpc::client(server_ip, rpc::constants::DEFAULT_PORT);
